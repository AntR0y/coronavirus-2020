...
class GMM():
    ...
    def _e_step(self, features):
        ...
        for k in range(self.n_clusters):
            if k == 0:
                posterior_probs = self._posterior(features, k).flatten()
            else:
                posterior_probs = np.column_stack((posterior_probs, self._posterior(features, k).flatten()))
    ...
    def _m_step(self, features, assignments):
        ...
        if self.covariance_type == 'spherical':
            covariances = np.zeros(self.n_clusters)
        else:
        ...
        for k in range(self.n_clusters):
            # calc big gamma
            big_gamma = np.sum(assignments[:,k])

            # 1. Update the means with the mu_j update in Slide 24.
            means[k,:] = np.sum((features.T * assignments[:,k]).T, axis=0) / big_gamma

            # 2. Update the mixing_weights with the w_j update in Slide 24
            mixing_weights[k] = big_gamma / features.shape[0]

            # 3. Update the covariance matrix with the sigma_j update in Slide 24.
            diffs = features - means[k,:]
            diffs_sq = (np.square(diffs).T * assignments[:,k]).T
            diffs_sq_sum = np.sum(diffs_sq, axis=0)
            if self.covariance_type == 'spherical':
                covariances[k] = np.mean(diffs_sq_sum / big_gamma)
            else:
                covariances[k,:] = diffs_sq_sum / big_gamma
    ...
    def _log_likelihood(self, features, k_idx):
        ...
        cova = np.array([])
        if self.covariance_type == 'spherical':
            cova = self.covariances[k_idx]
        else:
            cova = self.covariances[k_idx, :]
        a_log = multivariate_normal.logpdf(features, mean=self.means[k_idx], cov=cova)
        b_log = np.log(self.mixing_weights[k_idx])
        return b_log + a_log
